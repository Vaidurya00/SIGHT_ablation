...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # 确保 reward_model 字段不被移除，以便传递给 generate_sequences
            test_gen_batch = test_batch.pop(
                batch_keys=batch_keys_to_pop,
                non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
            )
            # 显式确保 reward_model 字段被传递（如果原始 test_batch 中有的话）
            if test_batch.non_tensor_batch and "reward_model" in test_batch.non_tensor_batch:
                if test_gen_batch.non_tensor_batch is None:
                    test_gen_batch.non_tensor_batch = {}
                test_gen_batch.non_tensor_batch["reward_model"] = test_batch.non_tensor_batch["reward_model"]
                logger.debug(f"Reward model field preserved in test_gen_batch: {len(test_batch.non_tensor_batch['reward_model'])} items")
            else:
                logger.warning("reward_model field not found in test_batch.non_tensor_batch. Ground truth may not be available for IGD.")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "tools_kwargs" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("tools_kwargs")
                # 确保 reward_model 字段不被移除，以便传递给 generate_sequences
                # reward_model 不在 non_tensor_batch_keys_to_pop 中，所以会被保留
                gen_batch = batch.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
                )
                # 显式确保 reward_model 字段被传递（如果原始 batch 中有的话）
                if batch.non_tensor_batch and "reward_model" in batch.non_tensor_batch:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            # Update global_step in reward_fn if it has the method
                            if hasattr(self.reward_fn, 'set_global_step'):
                                self.reward_fn.set_global_step(self.global_steps)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if reward_extra_infos_dict:
                            ...
                            # 提取三个独立的 score 并存储到 batch 中，用于后续计算 metrics
                            # 注意：这些 score 是序列级别的（每个样本一个值），需要转换为 token 级别
                            if "score_format" in reward_extra_infos_dict:
                                batch.non_tensor_batch["score_format"] = np.array(reward_extra_infos_dict["score_format"])
                            if "score_accuracy" in reward_extra_infos_dict:
                                batch.non_tensor_batch["score_accuracy"] = np.array(reward_extra_infos_dict["score_accuracy"])
                            if "score_mi" in reward_extra_infos_dict:
                                batch.non_tensor_batch["score_mi"] = np.array(reward_extra_infos_dict["score_mi"])
                        ...
                        if self.config.algorithm.use_kl_in_reward:
                            ...
                            # 如果 use_kl_in_reward=False，直接赋值（三个 score 之和已经在 reward_tensor 中）
