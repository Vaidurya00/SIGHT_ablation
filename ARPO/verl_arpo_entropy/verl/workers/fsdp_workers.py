        ...
        if rollout_name == "hf":
            ...
            if vllm_mode == "customized":
                ...
                if self.config.rollout.mode == "sync":
                    ...
                    # 用哪个 rollout 模块：优先从 config 读（由 sh 里 actor_rollout_ref.rollout.with_tools_module 传入，分布式时可靠），再回退到环境变量
                    import importlib
                    _tools_module = (
                        getattr(self.config.rollout, "with_tools_module", None)
                        or os.environ.get("ROLLOUT_WITH_TOOLS_MODULE")
                        or "vllm_rollout_with_tools"
                    )
                    _mod = importlib.import_module(
                        f"verl.workers.rollout.vllm_rollout.{_tools_module}"
                    )
                    vllm_rollout_cls = _mod.vLLMRolloutWithTools
                ...
                # 传递 worker 引用，以便 rollout 可以访问 ref 模型
                rollout_kwargs = {
                    ...
                    'trust_remote_code': trust_remote_code,
                    'worker': self,  # 传递 worker 引用
                    **lora_kwargs
                }
                rollout = vllm_rollout_cls(**rollout_kwargs)
            else:
