...
class vLLMRolloutWithTools(vLLMRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        training_step = prompts.meta_info.get('step', prompts.meta_info.get('global_step', None))  # 获取训练step
                ...
                for orig_sample, active_idxs in active_by_sample.items():
                    ...
                    for source_idx in active_idxs:
                        ...
                        for _ in range(branches_per_idx):
                            ...
                            # 如果prob大说明 entropy_delta小 说明tool-result带来的丰富性小
                            # if training_step == 1:
                            print(f"{training_step} -------------- random copy mechanism ------------------")
                            prob = random.random()
