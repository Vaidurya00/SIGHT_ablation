                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        if self.enable_igd:
                            ...
                            if orig_sample is not None and orig_sample < len(ground_truth_answers):
                                ...
                                # 策略一：负向约束分支（检测重复查询）- 最高优先级（不注入 hint prompt）
                                ...
                                # 策略二：反思性分支（MI低时）- 仅在策略一未触发时考虑（不注入 hint prompt）
            ...
            for i in range(batch_size):
                ...
                for idx in selected_indices:
                    ...
                    if self.enable_igd and idx in self.mi_scores and len(self.mi_scores[idx]) > 0:
                        ...
                        rollout_mi_avg = sum(rollout_mi_scores) / len(rollout_mi_scores)
                    else:
                        rollout_mi_avg = 0.0
                    rollout_mi_avgs.append(rollout_mi_avg)
