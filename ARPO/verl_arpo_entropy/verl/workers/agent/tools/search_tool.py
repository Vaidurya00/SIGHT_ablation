...
class BingSearchTool(BaseTool):
    ...
    def _extract_and_format_results(self, data: Dict) -> str:
        ...
        # Extract unique snippets (with URL/info/detail)
        chunk_content_list = []
        seen_snippets = set()
        for result in data['organic']:
            ...
            for field in [
                ...
                if isinstance(value, str):
                    value = value.strip()
                    if value and value not in content_sources:
                        content_sources.append(value)

            full_content = "\n".join(content_sources).strip()
            if not full_content:
    ...
    def _format_results(self, results: Dict) -> str:
        ...
        formatted = []
        for idx, item in enumerate(results["chunk_content"][:self._max_results], 1):
            if isinstance(item, dict):
                ...
                entry_lines = [f"Page {idx}:"]
                if title:
                    entry_lines.append(f"Title: {title}")

                preferred_text = content or snippet
                if preferred_text:
                    entry_lines.append(f"Content: {preferred_text}")
