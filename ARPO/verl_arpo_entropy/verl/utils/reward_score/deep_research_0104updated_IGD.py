...
def is_valid_sequence(text):
    '''
    is_valid_sequence 函数功能说明
    该函数用于验证文本是否符合预定义的标签序列格式，使用状态机检查标签顺序和完整性。
    主要功能模块

    2. 标签平衡检查（第97-103行）
    检查以下标签是否成对出现： think search result answer hint 统计每个标签的开闭数量，确保匹配 如果不匹配，返回错误信息

    3. <answer> 标签数量检查（第105-109行）
    要求 <answer> 标签必须且仅有一对
    如果数量不对，返回错误

    4. 状态机验证标签顺序（第111-161行）
    使用正则表达式分割内容，识别所有标签
    使用状态机严格验证标签出现的顺序
    允许的状态转换：
    start → think → after_think → search → after_search →   result → after_result → (hint → after_hint) → think → ... → answer → end
    标签之间只允许空白字符，不允许其他内容
    ...
    tags_to_check = ["think", "search", "result", "answer", "hint"]
    ...
    split_pattern = r"(</?(?:think|search|result|answer|hint)>)"
    parts = re.split(split_pattern, content)
    
    # 2. Keep track of the current position in the expected sequence
    state = "start"  # start -> think -> search -> result -> (hint) -> think -> ... -> answer -> end
    ...
    for i, part in enumerate(parts):
        ...
        if re.match(r"</?(?:think|search|result|answer|hint)>", part):
            # This is a tag, check if it's valid in the current state
            if part == "<think>" and state in ["start", "after_result", "after_hint"]:
                state = "in_think"
            ...
            if state in ["in_think", "in_search", "in_result", "in_answer", "in_hint"]:
                # Content is allowed inside tags
                pass
            elif state in ["start", "after_think", "after_search", "after_result", "after_hint"]:
