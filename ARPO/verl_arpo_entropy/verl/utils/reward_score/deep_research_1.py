...
def compute_score(data_source: str, solution_str: str, ground_truth: Any, extra_info: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    result = {
        ...
        "answer": "",
        "f1_score": 0,
        "base_score": 0.0  # 确保所有返回路径都包含 base_score
    }
    ...
    if not valid_template:
        ...
        result["base_score"] = 0.0  # 格式错误时 base_score 为 0
        return result
    ...
    if answer_part is None:
        ...
        result["base_score"] = 0.0  # 无法提取答案时 base_score 为 0
        return result
    ...
    try:
        ...
        result["base_score"] = 0.0  # 提取答案错误时 base_score 为 0
        return result
    ...
    # 添加MI reward增量 (R_total = R_outcome + λ * Σ Score_i)
    # 如果extra_info中包含mi_reward_increment，则将其加到base_score上
    mi_reward_increment = 0.0
    ...
    if extra_info is not None and "mi_reward_increment" in extra_info:
        mi_reward_increment = float(extra_info["mi_reward_increment"])
        result["mi_reward_increment"] = mi_reward_increment
        print(f"MI reward increment: {mi_reward_increment:.4f}")
    
    # 计算最终reward: R_total = R_outcome + λ * Σ Score_i
    final_score = base_score + mi_reward_increment
    result["base_score"] = base_score
    result["score"] = base_score
    result["reason"] += f" (base: {base_score:.4f},total: {base_score:.4f})"
